...
class ProductControllerTests {
    ...
    fun `we should create product`() {
        ...
        whenever(productService.createProduct("product name", 10000, time1, time2)).thenReturn(data)

        val resultActions: ResultActions = createProduct(1, "product name", 10000, time1, time2)
        resultActions.andExpect(status().isCreated)

        val content: String = resultActions.andReturn().response.contentAsString
        val product: ProductDTO = objectMapper.readValue(content)
    ...
    fun `we cannot create product without authentication`() {
        whenever(productService.createProduct("product name", 10000, time1, time2))
            .thenReturn(ProductDTO(1, "product name", 10000, 0, 0, time1, time2, false))
    ...
    fun `we should handle SQLException while creating product with database problem`() {
        whenever(productService.createProduct("product name", 1000, time1, time2))
            .thenAnswer { throw SQLException("error message") }
    ...
    fun `we should handle InvalidProductTitleException while creating product with invalid title`() {
        whenever(productService.createProduct("", 10000, time1, time2))
    ...
    fun `we should handle InvalidInvestingPeriodException while creating product with invalid investing period`() {
        whenever(productService.createProduct("product name", 10000, time2, time1))
    ...
    private fun createProduct(
        ...
        return mvc.perform(
            MockMvcRequestBuilders.post("/product")
                ...
                .param("title", title)
                .param("total_investing_amount", totalInvestingAmount.toString())
                .param("started_at", startedAt.toString())
                .param("finished_at", finishedAt.toString())
        )
    }
